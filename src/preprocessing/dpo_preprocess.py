...
def parse_cleaned_qna(cleaned_qna_str, q_id, logger):
    try:
        ...
        if logger:
            logger.warning(f"[JSONDecodeError] q_id: {q_id} - 응답 파싱 실패\n내용: {cleaned_qna_str}\n에러: {e}")
        return None
    except Exception as e:
        if logger:
            logger.warning(f"[UnknownError] q_id: {q_id} - 예기치 않은 오류 발생\n내용: {cleaned_qna_str}\n에러: {e}")
        return None
...
def cleaning(filtered_dpo_data_list, env, logger, batch_size):
    start_time = time.time()
    cleaned_data, start_idx = load_results(env["dpo_cleaned_data_path"])
    system_prompt = load_prompt(env["system_dpo_cleaning_prompt_path"])
    base_user_prompt = load_prompt(env["user_preprocessing_prompt_path"])
    
    filtered_data_to_process = filtered_dpo_data_list[start_idx:]
    ...
    for i, item in enumerate(tqdm(filtered_data_to_process, total=total_data_count, desc="Cleaning")):
        ...
        chosen_answer_type = item.get('chosen_answer_type', '')
        rejected_answer_type = item.get('rejected_answer_type', '')
        animal_type = item.get('animal_type', '')
        preprocessed_question = item.get('preprocessed_question', '')

        user_prompt = base_user_prompt.format(title=title, content=content, answer=rejected['answer'])
        cleaned_answer_str = call_gemini_api(system_prompt, user_prompt)
        parsed_result = parse_cleaned_qna(cleaned_answer_str, q_id, logger)
        
        if parsed_result is None:
            continue
        
        preprocessed_answer = parsed_result['preprocessed_answer']
        rejected['preprocessed_answer'] = preprocessed_answer
        ...
        cleaned_item = {
            ...
            "chosen": chosen,
            "rejected": rejected,
            "chosen_answer_type": chosen_answer_type,
            "rejected_answer_type": rejected_answer_type,
        ...
        if (i + 1) & batch_size == 0 or (i + 1) == total_data_count:
            ...
            save_json(cleaned_data, env["dpo_cleaned_data_path"])
            current_batch = []
...
if __name__ == "__main__":
    ...
    filtered_dpo_data_list = load_json(env["dpo_filtered_data_path"])
    logger.info(f"DPO FILTERED DATA PATH: {env['dpo_filtered_data_path']}")
    cleaning(filtered_dpo_data_list, env, logger, args.batch_size)
