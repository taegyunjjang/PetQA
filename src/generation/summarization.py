...
def get_prompts():
    system_prompt = """
    당신의 역할은 답변을 요약하는 것입니다.
    주어진 질문을 바탕으로 기존 답변의 의미와 핵심 정보를 유지하면서 요약하세요.
    """
...
def get_model_processor(model_name):
    return lambda *args: call_openai_api(model_name, *args)

def call_openai_api(model_name, *args):
...
def summarize_answers_batch(
    ...
    for item in data_to_process:
        system_prompt, base_user_prompt = get_prompts()
        user_prompt = base_user_prompt.format(question=item["preprocessed_question"], answer=item["preprocessed_answer"])
    ...
    batch_input_path = env["batch_input_path"].replace(".jsonl", f"_{args.model_name}.jsonl")
    ...
    batch_job = client.batches.create(
        ...
        metadata={"description": "summarizing answer"}
    )
    ...
    try:
        ...
        batch_output_path = env["batch_output_path"].replace(".jsonl", f"_{args.model_name}.jsonl")
        ...
        with open(batch_output_path, "r") as f:
            for line in f:
                ...
                content_dict[custom_id] = content

        for item in data_to_process:
            custom_id = f"{item['q_id']}_{item['a_id']}"
            item["summarized_answer"] = content_dict[custom_id]
...
def summarize_answers_sequential(
    args,
    data_to_process,
    results,
    output_path,
    ...
    for item in tqdm(data_to_process, total=len(data_to_process), desc="답변 요약 중"):
        system_prompt, base_user_prompt = get_prompts()
        user_prompt = base_user_prompt.format(question=item["preprocessed_question"], answer=item["preprocessed_answer"])
        summarized_answer = model_processor(system_prompt, user_prompt)
        item['summarized_answer'] = summarized_answer
        results.append(item)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_type", choices=["train", "validation", "test"], default="test")
    ...
    input_path = env[f"{args.file_type}_data_path"]
    output_path = f"./summarization_{args.file_type}.json"
    ...
    logger.info("Summarize answers completed.")