...
if __name__ == "__main__":
    ...
    classifier_model_path = os.path.join(env["checkpoint_dir"], "question_type_classifier", "best_model")
    ...
    logger.info("Classifying question types...")
    predictions = classifier(test_questions, batch_size=32, truncation=True, max_length=512)
    
    processed_results = []
    for i, test_sample in enumerate(tqdm(test_data, desc="Processing test samples")):
        predicted_question_type = predictions[i]['label']
    ...
    output_path = env["fewshot_examples_path"].replace(".json", "_bert.json")
