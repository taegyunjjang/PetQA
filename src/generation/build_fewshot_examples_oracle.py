...
def find_similar_samples(test_sample_embedding, question_type, categorized_train_embeddings, categorized_train_data_raw, k=6):
    train_embeddings_in_category = categorized_train_embeddings[question_type]['embeddings']
    train_data_in_category = categorized_train_data_raw[question_type]['data']
...
if __name__ == "__main__":
    ...
    output_path = env["fewshot_examples_path"].replace(".json", "_oracle.json")
    save_json(processed_results, output_path)
    logger.info(f"Few-shot examples saved: {output_path}")