"""PetQA 데이터셋으로 훈련한 오픈소스 모델을 사용하여 답변 생성 (vLLM 사용)"""
...
def get_prompts(model_name, shot, use_raw_format):
    ...
    if model_family == "claude":
        user_prompt_path = f"prompt/generate_answer/user_{model_family}{suffix}.txt"
...
def generate_answers_batch(llm, tokenizer, test_data, results, start_idx, output_path, use_raw_format, batch_size):
    prompts, data_to_process = prepare_prompts(test_data, start_idx, tokenizer, use_raw_format)
    ...
    for batch_idx in tqdm(range(total_batches), desc="배치 처리 중"):
        start_batch = batch_idx * batch_size
        end_batch = min((batch_idx + 1) * batch_size, len(prompts))
        
        batch_prompts = prompts[start_batch:end_batch]
        batch_data = data_to_process[start_batch:end_batch]
...
def generate_answers_sequential(llm, tokenizer, test_data, results, start_idx, output_path, use_raw_format, model_name, shot):
    prompts, data_to_process = prepare_prompts(test_data, start_idx, tokenizer, use_raw_format, model_name, shot)
    ...
    for i, (prompt, item) in enumerate(tqdm(zip(prompts, data_to_process), 
                                           total=len(prompts), 
                                           desc="답변 생성 중")):
        # 단일 프롬프트 생성
...
def main(model_name, use_finetuned_model, shot, use_raw_format, use_batch, batch_size):
    ...
    if use_batch:
        generate_answers_batch(llm, tokenizer, test_data, results, start_idx, env["output_path"], use_raw_format, batch_size)
    else:
        generate_answers_sequential(llm, tokenizer, test_data, results, start_idx, env["output_path"], use_raw_format, model_name, shot)
...
if __name__ == "__main__":
    ...
    main(args.model_name, args.use_finetuned_model, args.shot, args.use_raw_format, args.use_batch, args.batch_size)
